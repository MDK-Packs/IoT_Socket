...
The IoT Socket implementation is maintained in a public [GitHub repository](https://github.com/MDK-Packs/IoT_Socket). Its releases in [CMSIS Pack format](https://www.open-cmsis-pack.org/) are available under *MDK-Packs* on [CMSIS Packs page](https://developer.arm.com/tools-and-software/embedded/cmsis/cmsis-packs) and can be used in environments supporting the CMSIS-Pack concept.
...
Using \ref iot_socket_mux it is possible to retarget communication at run-time to either wired or wireless connections.

## Supported IoT clients {#iot_clients}

Typically, a user application does not call IoT Socket APIs directly, and relies on the IoT Client interface that manages the connection to the target cloud service provider (CSP).
...
[keil.com/iot](https://www2.keil.com/iot) references example projects that implement wired and wireless connectivity to common cloud services using extended IoT clients over IoT Socket interface. Figure below demonstrates the concept.
